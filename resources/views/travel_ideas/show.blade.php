...
@section('content')
@php
    $bgUrl = $travelIdea->image_url ?? 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=2073&auto=format&fit=crop';
@endphp
<div class="detail-hero" style="background-image: url('{{ $bgUrl }}');">
    <div class="container h-100 d-flex flex-column justify-content-end pb-5">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb bg-transparent p-0 mb-2">
                <li class="breadcrumb-item"><a href="{{ route('travel-ideas.index') }}" class="text-white-50 text-decoration-none">Browse</a></li>
                <li class="breadcrumb-item active text-white" aria-current="page">{{ $travelIdea->destination }}</li>
            </ol>
        </nav>
        <h1 class="display-3 text-white fw-bold mb-0">{{ $travelIdea->title }}</h1>
        <p class="text-white-50 fs-5 mb-5"><i class="bi bi-geo-alt-fill me-2"></i>{{ $travelIdea->destination }}</p>
    </div>
</div>
            ...
            <div class="card glass-card shadow-lg mb-4">
                <div class="card-body p-4 p-md-5">
                            ...
                            <div class="card border-0 bg-light h-100">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <span class="fs-5 fw-bold">{{ \Carbon\Carbon::parse($travelIdea->start_date)->format('M d') }} - {{ \Carbon\Carbon::parse($travelIdea->end_date)->format('M d, Y') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="card border-0 bg-light h-100">
                                <div class="card-body">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <span class="fs-5 fw-bold text-primary">${{ number_format($travelIdea->budget, 0) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <p class="card-text text-muted fs-5 leading-relaxed mb-4">
                        {{ $travelIdea->description ?? 'No detailed description available for this travel idea yet. Embark on this journey to discover the hidden gems and unique experiences awaiting in ' . $travelIdea->destination . '.' }}
                    </p>

                    <div class="mb-4">
                        <h5 class="fw-bold mb-3">Activities & Tags</h5>
                        <div class="d-flex flex-wrap gap-2">
                            @foreach(explode(',', $travelIdea->tags) as $tag)
                                <span class="badge rounded-pill bg-light text-primary border px-4 py-2 fs-6 shadow-sm">{{ trim($tag) }}</span>
                            @endforeach
                        </div>
                    </div>
                    ...
                    @if(isset($aviationData) && isset($aviationData['metar']))
                        @php $metar = $aviationData['metar']; @endphp
                        <div class="row g-4">
                            <div class="col-12">
                                <div class="card border-0 bg-soft-primary shadow-sm">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-center mb-3">
                                            ...
                                            <span class="badge bg-primary rounded-pill">{{ $metar['name'] ?? 'Local Station' }}</span>
                                        </div>
                                        <div class="row text-center g-3">
                                            <div class="col-3">
                                                <div class="small text-muted">Temperature</div>
                                                <div class="fw-bold fs-5">{{ $metar['temp'] ?? 'N/A' }}°C</div>
                                            </div>
                                            <div class="col-3">
                                                <div class="small text-muted">Flight Category</div>
                                                <div class="fw-bold fs-5">{{ $metar['fltCat'] ?? 'VFR' }}</div>
                                            </div>
                                            <div class="col-3">
                                                <div class="small text-muted">Wind Speed</div>
                                                <div class="fw-bold fs-5">{{ $metar['wspd'] ?? 0 }} kt</div>
                                            </div>
                                            <div class="col-3">
                                                <div class="small text-muted">Visibility</div>
                                                <div class="fw-bold fs-5">{{ $metar['visib'] ?? 'N/A' }} SM</div>
                                            </div>
                                        </div>
                                        <div class="mt-3 small text-muted text-end">
                                            <em>Observed: {{ $metar['reportTime'] ?? 'Recently' }}</em>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="card border-0 shadow-sm mb-5">
                <div class="card-body p-4 p-md-5">
                    <div class="d-flex justify-content-between align-items-center mb-4">
                        ...
                        <span class="badge bg-soft-primary px-3 py-2 rounded-pill">{{ $travelIdea->comments->count() }} Reviews</span>
                    </div>
                                ...
                                <div class="flex-grow-1">
                                    <div class="bg-light rounded-4 p-3 position-relative">
                                        <div class="d-flex justify-content-between align-items-start mb-1">
                                            ...
                                            <div class="text-warning small">
                                                @for($i = 0; $i < 5; $i++)
                                                    @php $isFilled = $i < $comment->rating; @endphp
                                                    <i class="bi bi-star{{ $isFilled ? '-fill' : '' }}"></i>
                                                @endfor
                                            </div>
                                        </div>
                                    ...
                                    <div class="mt-2 d-flex align-items-center" style="font-size: 0.85rem;">
                                        ...
                                        @auth
                                            ...
                                            @endphp
                                            <button class="btn btn-sm btn-link text-muted ms-3 p-0 px-2 interact-btn" data-type="like" data-id="{{ $comment->id }}"><i class="bi bi-hand-thumbs-up{{ $liked ? '-fill' : '' }}"></i> <span class="likes-count">{{ $comment->likes_count }}</span></button>
                                            <button class="btn btn-sm btn-link text-muted p-0 px-2 interact-btn" data-type="dislike" data-id="{{ $comment->id }}"><i class="bi bi-hand-thumbs-down{{ $disliked ? '-fill' : '' }}"></i> <span class="dislikes-count">{{ $comment->dislikes_count }}</span></button>
                                            <button class="btn btn-sm btn-link text-muted p-0 px-2 reply-toggle-btn" data-id="{{ $comment->id }}">Reply</button>
                                        @else
                                            <span class="text-muted ms-3 p-0 px-2"><i class="bi bi-hand-thumbs-up"></i> {{ $comment->likes_count }}</span>
                                            <span class="text-muted p-0 px-2"><i class="bi bi-hand-thumbs-down"></i> {{ $comment->dislikes_count }}</span>
                                        @endauth
                                    </div>

                                    <!-- Reply Form -->
                                    <div class="reply-form mt-2" id="reply-form-{{ $comment->id }}" style="display:none;">
                                        <form class="nested-comment-form" data-url="{{ route('comments.store', $travelIdea->id) }}">
                                            @csrf
                                    ...
                                    <!-- Nested Replies -->
                                    <div class="replies-container mt-3 ms-2" id="replies-{{ $comment->id }}">
                                        @foreach($comment->replies()->with('user')->latest()->get() as $reply)
                                            <div class="d-flex mb-3 comment-item" data-id="{{ $reply->id }}">
                                                ...
                                                <div class="flex-grow-1">
                                                    <div class="bg-light rounded-4 p-2">
                                                        <h6 class="fw-bold mb-0" style="font-size:0.9rem;">{{ $reply->user->name }}</h6>
                                                        <p class="text-muted mb-0 small">{!! $reply->content !!}</p>
                                                    </div>
                                                    <div class="mt-1 d-flex align-items-center" style="font-size: 0.75rem;">
                                                        ...
                                                        @auth
                                                            ...
                                                            @endphp
                                                            <button class="btn btn-sm btn-link text-muted ms-2 p-0 px-2 interact-btn" data-type="like" data-id="{{ $reply->id }}"><i class="bi bi-hand-thumbs-up{{ $rLiked ? '-fill' : '' }}"></i> <span class="likes-count">{{ $reply->interactions()->where('type', 'like')->count() }}</span></button>
                                                            <button class="btn btn-sm btn-link text-muted p-0 px-2 interact-btn" data-type="dislike" data-id="{{ $reply->id }}"><i class="bi bi-hand-thumbs-down{{ $rDisliked ? '-fill' : '' }}"></i> <span class="dislikes-count">{{ $reply->interactions()->where('type', 'dislike')->count() }}</span></button>
                                                        @else
                                                            <span class="text-muted ms-2 p-0 px-2"><i class="bi bi-hand-thumbs-up"></i> {{ $reply->interactions()->where('type', 'like')->count() }}</span>
                                                            <span class="text-muted p-0 px-2"><i class="bi bi-hand-thumbs-down"></i> {{ $reply->interactions()->where('type', 'dislike')->count() }}</span>
                                                        @endauth
                                                    </div>
                                                </div>
                                            </div>
                    ...
                    @auth
                        <div class="bg-light rounded-4 p-4">
                            ...
                            <form id="comment-form" data-url="{{ route('comments.store', $travelIdea->id) }}">
                                @csrf
                                <div class="row g-3">
                                    <div class="col-md-4">
                    ...
                    @else
                        <div class="text-center py-4 bg-light rounded-4">
                            <p class="mb-0">Ready to join the conversation? <a href="{{ route('login') }}" class="fw-bold text-decoration-none">Login</a> to leave your review.</p>
                        </div>
                    @endauth
                </div>
            </div>
            ...
            <div class="sticky-top" style="top: 100px; z-index: 5;">
                <div class="card book-now-card border-0 shadow-lg mb-4">
                    <div class="card-body p-4 text-white">
                        ...
                        <p class="card-text opacity-75 mb-4 text-white">Explore top-rated hotel recommendations around {{ $travelIdea->destination }} today!</p>
                        <a href="{{ route('travel-ideas.hotels', $travelIdea->id) }}" class="btn btn-primary w-100 fw-bold shadow-lg">
                    ...
                    <div class="card border-0 shadow-sm overflow-hidden">
                        ...
                        <div class="card-body p-4">
                            <div class="d-grid gap-2">
                                <a href="{{ route('travel-ideas.edit', $travelIdea->id) }}" class="btn btn-warning">
                                    <i class="bi bi-pencil-square me-2"></i>Edit Trip
                                </a>
                                <form action="{{ route('travel-ideas.destroy', $travelIdea->id) }}" method="POST" class="d-grid">
                                    @csrf
                                    @method('DELETE')
...
$(document).ready(function() {
    ...
    const fetchUrl  = "{{ route('comments.latest', $travelIdea->id) }}";
